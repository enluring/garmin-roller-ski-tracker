...
// Accelerometer axis selector, matching the "axis*" properties. (SkiType
// itself is defined in RollerSkiTrackerApp.mc.)
module AccelAxis {
    const X = 0;
    const Y = 1;
    const Z = 2;
...
class StrokeDetector {
    ...
    // Call after Application.Properties may have changed (a threshold/axis
    // recalibrated from the phone) so the detector picks up the new config
    ...
    // Intended to be polled once per second (RollerSkiTrackerApp.tickStrokes()).
