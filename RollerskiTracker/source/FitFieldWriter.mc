...
class FitFieldWriter {
    ...
    // `owner` is the WatchUi.DataField instance (createField() is inherited
    // from DataField and must be called on it, not on this helper).
    ...
    // Call once per DataField.compute() tick with the latest StrokeStats.
